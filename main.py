...
def assigne_lecture(className, subjectName, teacher_id, total_lecture, break_period, duration, start_time):
            ...
            for period in range(1, len(periods) - 1):  
                ...
                for room_id in roomList:
                    ...
                    if all(
                        ...
                        end_time = datetime.strptime(start_time_str, "%I:%M %p") + timedelta(minutes=lab_duration)
                        time_range = f"{start_time_str} - {end_time.strftime('%I:%M %p')}"
    ...
    for day in days:
        ...
        for period in periods:
            ...
            for room_id in roomList:
                if (
                    timetable[className][day][period] is None and
                    teacher_availble(teacher_id, day, period) and
                    room_available(room_id, day, period)
                ):
...
def generate_timable():
    ...
    teacher_usage = {
        tid: {day: {p: False for p in periods} for day in days}
        for tid in teacher
    }

    room_usage = {
        rid: {day: {p: False for p in periods} for day in days}
        for rid in room["normal_rooms"] + room["lab_rooms"]
    }
    ...
    for (cls, sub), widgets in subjects.items():
        ...
        if tid and matched_sub:
            ...
            lectures_required = 1 if subType == "lab" else subject[matched_sub].get("weekly_lectures", 3)
                ...
                for alt_day in retry_days:
                    for alt_period in periods:
                        for alt_room in room["lab_rooms"] if subType == "lab" else room["normal_rooms"]:
                            if subType == "lab":
                                valid_lab = all(
                                    timetable[cls][alt_day].get(p) is None and
                                    teacher_availble(tid, alt_day, p) and
                                    room_available(alt_room, alt_day, p)
                                    for p in range(alt_period, alt_period+3)
                                    if p in periods and p != break_period
                                )
                                if valid_lab:
                                    for p in range(alt_period, alt_period+3):
                                        if p in periods:
                                            time_range = time_slot(alt_period, start_time, duration)
                                            timetable[cls][alt_day][p] = {
                                                "subject": matched_sub + " (lab)",
                                                "teacher": teacher[tid]["name"],
                                                "room": alt_room,
                                                "time": time_range
                                            }
                                            mark_usage(tid, alt_room, alt_day, p)
                                    success = True
                                    break
                            else:
                                if (alt_period != break_period and
                                    timetable[cls][alt_day][alt_period] is None and
                                    teacher_availble(tid, alt_day, alt_period) and
                                    room_available(alt_room, alt_day, alt_period)):
                                    timetable[cls][alt_day][alt_period] = {
                                        "subject": matched_sub,
                                        "teacher": teacher[tid]["name"],
                                        "room": alt_room,
                                        "time": time_slot(alt_period, start_time, duration)
                                    }
                                    mark_usage(tid, alt_room, alt_day, alt_period)
                                    success = True
                                    lectures_required -= 1
                    ...
                    if success:
                        break
            if not success:
                skipped_subjects.append(f"{cls}-{matched_sub}")
        else:
            skipped_subjects.append(f"{cls}-{sub}")
    ...
    if skipped_subjects:
        result += "\n⚠️ Skipped (could not assign): " + ", ".join(skipped_subjects)
...
frames=[setup_frame,timetable_fame,home_frame,teacher_frame,room_frame,class_frame,subjects_frame]
...
def show_teachers_data():
    ...
    ctk.CTkButton(teacher_frame, text="⬅️ Back to Home", command=lambda: showframe(home_frame),**button_config).pack(pady=10)
...
def show_class_data():
    ...
    ctk.CTkButton(class_frame, text="⬅️ Back to Home", command=lambda: showframe(home_frame),**button_config).pack(pady=10)
...
def show_subject_data():
    ...
    ctk.CTkButton(subjects_frame, text="⬅️ Back to Home", command=lambda: showframe(home_frame), **button_config).pack(pady=10)
...
def show_rooms_data():
    ...
    ctk.CTkButton(room_frame, text="⬅️ Back to Home", command=lambda: showframe(home_frame),**button_config).pack(pady=10)
